...
class MockCuiEmbedding: # Simplified from your version for focus
    def __init__(self, embedding_data_dict, hdim, device):
        self.data = {cui: emb.clone().detach().to(device) for cui, emb in embedding_data_dict.items()}
        self.hdim = hdim
        self.device = device
    def encode(self, cui_str_list: list):
        embs = [self.data.get(c, torch.zeros(self.hdim, device=self.device)) for c in cui_str_list]
        return torch.stack(embs) if embs else torch.empty(0, self.hdim, device=self.device)

class MockEdgeOneHot: # Simplified from your version for focus
    def __init__(self, graph, *args, **kwargs):
        self.edge_labels = sorted(list(set(d.get('label', 'UNKNOWN') for _, _, d in graph.edges(data=True))))
        self.edge_to_idx = {lbl: i for i, lbl in enumerate(self.edge_labels)}
        self.num_edge_types = len(self.edge_labels) if self.edge_labels else 1
        self.onehot_mat = F.one_hot(torch.arange(self.num_edge_types)).float() if self.num_edge_types > 0 else torch.zeros(0,1).float()
    def Lookup(self, labels_str):
        if self.num_edge_types == 0: return torch.empty(len(labels_str), 0)
        indices = torch.tensor([self.edge_to_idx.get(lbl, 0) for lbl in labels_str], dtype=torch.long)
        return self.onehot_mat[indices] if indices.numel() > 0 else torch.empty(0, self.num_edge_types)
...
class GraphModel(nn.Module):
    ...
    def one_iteration(self,
        ...
        pruning_threshold_count = 2048 # 您設定的篩選路徑數量上限
...
class TestGraphModelPathPruning(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        print("\n--- Preparing Test Environment for Path Pruning (setUpClass) ---")
        cls.hdim = 128 
        cls.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        print(f"TestGraphModelPathPruning using device: {cls.device}")

        # 1. 創建圖譜
        cls.num_target_nodes_for_pruning_test = 3000
        cls.g_nx = nx.DiGraph()
        cls.source_cui_pruning = "C_SOURCE"
        cls.target_cuis_pruning = [f"C_TGT_{i}" for i in range(cls.num_target_nodes_for_pruning_test)]
        cls.all_mock_nodes = [cls.source_cui_pruning] + cls.target_cuis_pruning
        
        for i in range(cls.num_target_nodes_for_pruning_test):
            cls.g_nx.add_edge(cls.source_cui_pruning, f"C_TGT_{i}", label=f"R{i%5}")

        # 2. 定義 task_emb (必須在創建 mock_embedding_data 之前，並且在正確的設備上)
        cls.task_emb_for_pruning = torch.randn(1, cls.hdim, device=cls.device) # Shape [1, hdim]

        # 3. 創建 Mock CUI 嵌入數據
        cls.mock_embedding_data = {}
        for i, cui_str in enumerate(cls.all_mock_nodes):
            if cui_str == cls.source_cui_pruning:
                cls.mock_embedding_data[cui_str] = torch.randn(cls.hdim, device=cls.device)
            else: 
                # 確保所有用於計算的張量都在 cls.device 上
                random_noise = torch.randn(cls.hdim, device=cls.device) * 0.1
                base_emb = cls.task_emb_for_pruning.squeeze(0) # Shape [hdim]
                
                if i < cls.num_target_nodes_for_pruning_test // 2 : 
                    combined_emb = base_emb + random_noise # Shape [hdim]
                else: 
                    # 為了產生與 task_emb 明顯不同的嵌入，可以不加 base_emb 或用不同的基向量
                    combined_emb = torch.randn(cls.hdim, device=cls.device) # Shape [hdim]
                
                # ## MODIFIED: 修正 renorm 的使用 ##
                # unsqueeze(0) -> [1, hdim], renorm on dim=1, squeeze(0) -> [hdim]
                cls.mock_embedding_data[cui_str] = combined_emb.unsqueeze(0).renorm(p=2, dim=1, maxnorm=1.0).squeeze(0)
        
        # 4. 實例化 MockCuiEmbedding
        cls.mock_cui_lookup = MockCuiEmbedding(cls.mock_embedding_data, cls.hdim, cls.device)

        # 5. 預處理圖譜為張量 (傳入 cls.device)
        ...
        print(f"Mock graph for pruning test edge_to_idx: {cls.e2i}")
        print(f"Number of nodes in tensor_graph: {cls.tensor_graph['num_nodes']}")
        print(f"Number of edges in tensor_graph: {cls.tensor_graph['adj_src'].numel()}")


    # ... (您的 assertTensorsOnDevice 和 test_05_path_pruning_logic 方法) ...
    # 確保在 test_05_path_pruning_logic 中實例化 GraphModel 時，
    # 使用 cls.g_nx, cls.mock_cui_lookup, cls.hdim, cls.device 等 setUpClass 中定義的屬性

    # 輔助方法 (假設您會將其加入到 TestGraphModelPathPruning 類中)
    def assertTensorsOnDevice(self, tensor_dict, device, dict_name="Tensor Dict"):
        if tensor_dict is None: return
        for key, tensor_val in tensor_dict.items(): # 修改變量名以避免與外部 torch.tensor 混淆
            if isinstance(tensor_val, torch.Tensor):
                self.assertEqual(tensor_val.device.type, device.type, 
                                 f"Tensor '{key}' in '{dict_name}' is not on expected device {device} (is on {tensor_val.device})")

    def test_05_path_pruning_logic(self): # 您的測試方法保持不變
        print("\n--- Test 5: Path Pruning in one_iteration ---")
        
        pruning_threshold_count_in_model = 2048 
        
        # 假設 GraphModel, EdgeOneHot, MockPathEncoder, MockPathRanker 都已正確定義或導入
        # GraphModel 的 __init__ 需要使用 setUpClass 中準備好的 cls.g_nx
        # 它的 EdgeOneHot 也會基於 cls.g_nx 初始化
        model = GraphModel(
            g_nx=self.g_nx, # 使用 setUpClass 中創建的圖
            ...
            path_encoder_type="MLP", 
            gnn_update=False,
            # 提供 GraphModel __init__ 所需的所有其他參數
            # 例如: cui_weights_dict, path_ranker_type, gnn_type, gin_hidden_dim, gin_num_layers
            cui_weights_dict=None,
            path_ranker_type="Flat", # 或 "Combo"
            gnn_type="Stack", # 或 "Single"
            gin_hidden_dim=self.hdim,
            gin_num_layers=1
        )
        # 替換為 Mock (如果 GraphModel 內部沒有這樣做)
        
        model.to(self.device)

        print(f"  Testing with initial path count: {self.num_target_nodes_for_pruning_test} (should trigger pruning)")
        all_paths_info_pruned, _, next_hop_state_pruned, _ = model.one_iteration(
            task_emb_batch=self.task_emb_for_pruning, 
            current_cui_str_list=[self.source_cui_pruning],
            running_k_hop=0,
            prev_iteration_state=None
        )

        self.assertIsNotNone(all_paths_info_pruned, "all_paths_info_pruned should not be None")
        
        num_paths_after_pruning = all_paths_info_pruned['scores'].shape[0]
        self.assertEqual(num_paths_after_pruning, pruning_threshold_count_in_model,
                         f"Number of paths after pruning should be {pruning_threshold_count_in_model}, got {num_paths_after_pruning}")

        for key in ['src_idx', 'tgt_idx', 'edge_idx', 'encoded_embeddings']:
            self.assertEqual(all_paths_info_pruned[key].shape[0], pruning_threshold_count_in_model,
                             f"Tensor all_paths_info_pruned['{key}'] length mismatch after pruning.")
        if all_paths_info_pruned.get('mem_orig_src_idx') is not None:
             self.assertEqual(all_paths_info_pruned['mem_orig_src_idx'].shape[0], pruning_threshold_count_in_model)
        if all_paths_info_pruned.get('mem_first_edge_idx') is not None: # 檢查這個新添加的鍵
             self.assertEqual(all_paths_info_pruned['mem_first_edge_idx'].shape[0], pruning_threshold_count_in_model)
        
        print(f"PASS: Pruning correctly reduced path count to {pruning_threshold_count_in_model}.")

        print(f"  Testing with initial path count: 0 (should not trigger pruning, or handle empty gracefully)")
        all_paths_info_no_prune, _, _, _ = model.one_iteration(
            task_emb_batch=self.task_emb_for_pruning,
            current_cui_str_list=["C_NON_EXISTENT"], 
            running_k_hop=0,
            prev_iteration_state=None
        )
        self.assertIsNone(all_paths_info_no_prune, "Should return None if no paths are found from start.")
        print("PASS: No pruning applied and handled gracefully when initial path count is 0.")
        
        if all_paths_info_pruned is not None:
            self.assertTensorsOnDevice(all_paths_info_pruned, self.device, "all_paths_info_pruned")
        if next_hop_state_pruned is not None:
            self.assertTensorsOnDevice(next_hop_state_pruned, self.device, "next_hop_state_pruned")


if __name__ == '__main__':
    # 確保您的 GraphModel 和其他依賴項在此處已定義或導入
    # 例如：
    # from your_script import GraphModel, MockPathEncoder, MockPathRanker, EdgeOneHot 
    # from your_script import preprocess_graph_to_tensors, retrieve_neighbors_paths_no_self_tensorized
    # from your_script import CuiEmbedding, GINStack, NodeAggregateGIN, MLP, PathEncoderTransformer, TriAttnFlatPathRanker, TriAttnCombPathRanker


    # 這裡需要您之前粘貼的完整 GraphModel 和其所有依賴項定義
    # (確保 MockCuiEmbedding 和 preprocess_graph_to_tensors 也使用上面測試類中的版本)
    
    suite = unittest.TestSuite()
    suite.addTest(unittest.makeSuite(TestGraphModelPathPruning))
